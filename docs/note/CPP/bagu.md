...
- **如何保证类的对象只能被开辟在堆上？（将构造函数声明为私有、单例）**
  ...
  答：将构造函数设置为私有，这样只能使用`new`运算符来建立对象，但是我们必须准备一个`destory`函数来进行内存的释放，然后将析构函数设置为`protected`，提供一个`public`的`static`函数来完成构造，类似于单例模式。
...
- **假如类D继承了类B，且析构函数均不是虚函数，那么shared<B>(new D)能正确析构吗？**
...
- **静态变量在哪里初始化？在哪一个阶段初始化？（都存放在全局区域）**
  ...
  答：静态变量，全局变量，常量都在编译阶段完成初始化和内存分配。其他变量都是在编译阶段进行初始化，运行阶段内存分配。
...
- **define 宏定义 和 const 的区别**
  ...
  2. 宏定义**只做替换** ，**不会进行检查，const **有数据类型，**编译器会对它进行**类型检查
...
- **auto是怎么实现自动识别类型的？模板是怎样实现转化成不同类型的？**
